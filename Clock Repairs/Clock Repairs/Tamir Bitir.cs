...
{
    public partial class Tamir_Bitir : Form
    {
        public Tamir_Bitir()
        {
            InitializeComponent();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                Sabit_Kodlar.conn.Open();
                SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT * FROM Tamir_Giris where İsim like '%" + textBox7.Text + "%'", Sabit_Kodlar.conn);
                DataTable dataTable = new DataTable();
                dataAdapter.Fill(dataTable);
                dataGridView1.DataSource = dataTable.DefaultView;
                Sabit_Kodlar.conn.Close();
            ...
            {
                Sabit_Kodlar.conn.Open();
                SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT * FROM Tamir_Giris where Soyisim like '%" + textBox7.Text + "%'", Sabit_Kodlar.conn);
                DataTable dataTable = new DataTable();
                dataAdapter.Fill(dataTable);
                dataGridView1.DataSource = dataTable.DefaultView;
                Sabit_Kodlar.conn.Close();
            ...
            {
                Sabit_Kodlar.conn.Open();
                SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT * FROM Tamir_Giris where Telefon like '%" + textBox7.Text + "%'", Sabit_Kodlar.conn);
                DataTable dataTable = new DataTable();
                dataAdapter.Fill(dataTable);
                dataGridView1.DataSource = dataTable.DefaultView;
                Sabit_Kodlar.conn.Close();
            ...
            {
                Sabit_Kodlar.conn.Open();
                SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT * FROM Tamir_Giris where Tamir_Edilecek_Esya like '%" + textBox7.Text + "%'", Sabit_Kodlar.conn);
                DataTable dataTable = new DataTable();
                dataAdapter.Fill(dataTable);
                dataGridView1.DataSource = dataTable.DefaultView;
                Sabit_Kodlar.conn.Close();
            ...
            {
                Sabit_Kodlar.conn.Open();
                SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT * FROM Tamir_Giris where Model like '%" + textBox7.Text + "%'", Sabit_Kodlar.conn);
                DataTable dataTable = new DataTable();
                dataAdapter.Fill(dataTable);
                dataGridView1.DataSource = dataTable.DefaultView;
                Sabit_Kodlar.conn.Close();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox6.Text == "")
            {
                MessageBox.Show("LÜTFEN ÖDENCEK TUTARI DOLDURUNUZ");
            }
            else if (textBox1.Text == "")
            {
                MessageBox.Show("LÜTFEN TABLODAN MÜŞTERİ SEÇİNİZ");
            }
            else
            {
                Sabit_Kodlar.conn.Open();

                SqlCommand comm = new SqlCommand("insert into Tamir_Bitir(İsim,Soyisim,Telefon,Tamir_Edilecek_Esya,Model,Tamir_Sebebi,Tahmini_Ucret,Tarih_Giris,Tamir_Turu,Gonderilen_Firma,Odenmesi_Gereken_Ucret,Resim) values (@name,@lastname,@number,@tee,@model,@ts,@tu,@tg,@tt,@gf,@ou,@resim)", Sabit_Kodlar.conn);
                ...
                comm.Parameters.AddWithValue("@resim", dataGridView1.CurrentRow.Cells[11].Value.ToString());
                ...
                fill_table();
                clear();
                MessageBox.Show("TAMİR BİTTİ ");
            }
        }

        private void Tamir_Bitir_Load(object sender, EventArgs e)
        {
            fill_table();
